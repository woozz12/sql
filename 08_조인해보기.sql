...
FROM
    employees   e,
    departments d -- as 생략 가능
WHERE
    e.department_id = d.department_id; -- 오라클 조인
...
FROM
         employees e
    INNER JOIN departments d ON e.department_id = d.department_id; --ANST 표준 조인 문법

/*
각각의 테이블에 독립적으로 존재하는 컬럼의 경우에는
테이블 이름을 생략해도 무방합니다. 그러나 해석의 명확성을 위해
테이블 이름을 작성하셔서 소속을 표현해 주는 것이 바람직합니다.
테이블 이름이 너무 길 시에는 ALLAS를 작성하여 칭합니다.
...
/*
loc 체이블의 province = 'california' 조건에 맞는 값을 대상으로
2. location_id 값과 같은 값을 가지는 데이터를
    departments 에서 찾아서 조인
3. 위에 결과와 동일한 department_id 가진
   employees 테이블의 테이터를 찾아서 조인
...
FROM
    ...
    locations loc
WHERE
    e.department_id = d.department_id(+) -- (+)가 안 붙은 쪽이 기준이 된다.
