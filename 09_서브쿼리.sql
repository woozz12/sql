...
WHERE job_id = (SELECT job_id 
                FROM employees 
                WHERE job_id ='IT_FROG'); -- 에러
        
--다중행 연산자
-- IN: 목록에 어떤 값과 같은 지 확인합니다. 일치하는 값들을 조회해줌
...
-- salary로 정렬을 진행하면서 바로 ROWNIM을 붙이면
-- ROWNUM이 정렬이 되지 않는 상황이 발생합니다.
-- 이유: ROWNUM이 먼저 붙고 정렬이 진행되기 때문에 ORDER BY는 항상 마지막에 진행
...
SELECT * FROM 
    (
    SELECT
-- TO_CHAR 문자열로 변경 MMDD   TO_DATE는 날짜 타입으로 변경  day,YY/MM/DD
        TO_CHAR(TO_DATE(day,'YY/MM/DD'), 'MMDD') AS mm, name
    FROM
        (
        SELECT '홍길동' AS name, '20230418' AS day FROM dual UNION ALL -- 합집합
        SELECT '김철수' , '20230301' FROM dual UNION ALL
        SELECT '박영희' , '20230701' FROM dual UNION ALL
        SELECT '김뽀삐' , '20230401' FROM dual UNION ALL
        SELECT '박뚜띠' , '20230501' FROM dual UNION ALL
        SELECT '감스트' , '20230601' FROM dual
        )
    )
WHERE mm = '0418'; -- 내가 필요한 조건
