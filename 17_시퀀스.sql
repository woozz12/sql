...
-- 시퀀스 생성 에러에 위험이 있다면 사용 X
CREATE SEQUENCE dept3_seq 
    START WITH 1 -- 시작값 (기본값은 증가할 때 최소값, 감소할 때 최대값) 100시작
    INCREMENT BY 1 -- 증가값 (양수면 증가, 음수면 감소, 기본값 1)   -1 감소
    MAXVALUE 10 -- 최대값(기본값은 증가 1027, 감소일 때 -1)
    MINVALUE 1 -- 최소값(기본값 증가일 때 1 , 감소일 때 -1028)
    NOCACHE -- 캐시메모리 사용 여부(CACHE) 
    NOCYCLE; -- 순환 여부(NOCYCLE이 기본, 순환시키려면 CYCLE) 10까지 도달 후 끝    
...
DROP SEQUENCE dept3_seq; -- 시퀀스 삭제
